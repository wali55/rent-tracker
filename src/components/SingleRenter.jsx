...
const SingleRenter = ({ renter }) => {
  ...
  return (
    <tr className="text-left text-neutral-300 text-sm hover:bg-cyan-950 cursor-pointer">
      ...
      <td className="p-3">
        {lastPaymentDate
          ? new Intl.DateTimeFormat("en-GB", {
              ...
              month: "long",
              year: "numeric",
            }).format(new Date(lastPaymentDate + "T00:00:00"))
          : "-"}
      </td>
      <td className="p-3">{dueAmount}</td>
      <td className="p-3">
        {status === "paid" ? (
          <div className="bg-lime-700 text-center rounded-full w-fit px-2 py-0.5 text-xs font-medium">
            পরিশোধিত
          </div>
        ) : (
      ...
      <td className="p-3 flex justify-center relative">
        <button
          ...
          className="border w-fit px-2 py-1 rounded-md hover:border-cyan-300 hover:text-cyan-300 cursor-pointer"
        >
          <BsThreeDots />
        </button>
